...
| Any theme switch, either direction | The log has `SerializerCleanCopies: dropped N clean copies` right after the switch; N is 0 until a Vision save has happened since the last switch | — | — | #92 part 1. The cache the gate exists for is emptied at each switch; nothing to see on screen until parts 2 and 3 land |
