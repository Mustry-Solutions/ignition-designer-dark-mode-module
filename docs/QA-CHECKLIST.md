...
| Drop a Rectangle and a Tree View under dark, save; open in a Vision client | The rectangle's text colour is the normal dark grey, not near-white; the tree's sample rows have white backgrounds and dark text | — | — | The two leaks the palette sweep found. A byte scan of the save for `-2236963` (#DDDDDD) and `color(70,73,75` is the quick check |
