...
| Any theme switch, either direction | The log has `SerializerCleanCopies: dropped N clean copies` right after the switch; N is 0 until a Vision save has happened since the last switch | `pass` | `2026-09-17` | #92 part 1. The cache the gate exists for is emptied at each switch. 2026-09-17: `dropped 13 clean copies: [BasicContainer, DefaultInteractionController, FPMIWindow, …]` on the dark switch after the Vision saves |
| Light Designer. Drop a Button and a Multi-State Indicator from the palette onto a window, save | `ops/vision-check.sh` clean; the saved XML is identical to one from a Designer without the module (no colour calls on the button; the indicator's state dataset carries the stock colours) | `pass` | `2026-09-17` | #92 part 2. The module's Color delegate is on every save, light or dark, and must be invisible while light. 2026-09-17: Check Box and Multi-State Indicator (a Krisp overlay covered the Button); checker clean, no colour calls added. Nothing in this row is dark; the dark half needs the gate gone |
| Dark Designer on Perspective. Navigate to Vision (drop-out), open a window with a Text Field, look at the field's font and at any Designer text field's font, then save | Both fonts are the Designer's usual one, not a smaller Tahoma; the log has a `primeStyles` phase in the restore trace; `ops/vision-check.sh` clean, no `setFont` | `partial` | `2026-09-17` | #92 part 3. Synthetica's first formatted-text-field style after a reinstall is stale; the restore now spends it on a throwaway component. 2026-09-17: the restore ran with no failed phase (the trace itself is not logged; only a failure would be) and the checker found no `setFont`, but the field's font could not be read from the property editor (#102) and the field is empty on screen. Visible only once a Vision component has been created under dark, so with the gate this row mostly checks the phase runs |
