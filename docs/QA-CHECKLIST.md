...
| Continue from the row above without touching the menu: quit (or File → Open another project) while still on Vision | No dialog on the way out; the log has no `Vision gate: Dark mode was not applied…` line at shutdown; the next launch away from Vision comes up dark | — | — | The Designer rebuilds module menus at teardown; the seeded checkbox must not read as a click |
