...
- **Reflection into the running Designer.** To reach colors the Ignition UI
  ...
  confined to the Designer's own UI objects, not user project content: no
  reflective pass runs inside the Vision design canvas, and Vision saves are
  guarded so the dark theme is not written into a saved window. The canvas
  does render dark, because the look and feel is global (see the README's
  [Known limitations](README.md#known-limitations)). Mutations are tracked
  and reversed when dark mode is turned off. A relaunch always starts from
  the stock theme, so it is the sure way to clear anything a toggle-off
  missed.
